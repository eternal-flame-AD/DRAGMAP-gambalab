...
Help()
{
   # Display Help
	...
	echo
	echo "Syntax: run_dragen.sh [h|s|1|2|o|r|c|t|b|d]"
    	echo "options:"
    	echo "-h     Print this Help."
    	echo "-c     Number of cpus to use. (Required)"
    	echo "-o     Output directory. (Required)"
    	echo "-s     Sample name. (Required)"
    	echo "-1     Path to the read1 FASTQ. (Required)"
    	echo "-2     Path to the read2 FASTQ. (Required)"
    	echo "-r     Path to the Dragmap reference folder. (Required)"
    	echo "-t     Trimming. Default false. (Optional)"
      echo "-b     BED file with regions will be used to compute coverage. Otherwise coverage stats are computed whole genome. (Optional)"
	   echo "-d     Delete trimmed fastQ. Default false. (Optional)"
    	echo
}
